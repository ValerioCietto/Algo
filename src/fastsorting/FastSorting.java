...
public class FastSorting {
	...
	private static void MergeSortWithSingleAuxiliaryArray(int[] a, int fst,
			int lst, int[] aux) {
		if (fst < lst) {
			int m = (fst + lst) / 2;
	...
	private static void MergeSortJavaAPIVersion(int[] a, int fst, int lst,
			int[] b) {
		if (fst < lst) {
			int m = (fst + lst) / 2;
	...
	private static void MergeJavaAPIVersion(int[] a, int fst, int mid, int lst,
			int[] b) {
		int i = fst, j = mid + 1, k = fst;
		while (i <= mid && j <= lst) {
			if (a[i] <= a[j])
	...
	/**
	 * Scambia gli elementi i e j nell'array a. i e j devono essere compresi tra
	 * 0 e a.length
	 * 
	 * @param a
	 *            array su cui viene eseguito lo scambio
	 * @param i
	 *            primo indice
	 * @param j
	 *            secondo indice
